...
class TushareAdapter(DataSourceAdapter):
    ...
    def _get_default_priority(self) -> int:
        return 3  # highest priority (数字越大优先级越高)
    ...
    def query(self, api_name: str, **kwargs) -> Optional[pd.DataFrame]:
        """Generic query to tushare pro api"""
        if not self.is_available():
            return None
        try:
            api = self._provider.api
            if hasattr(api, "query"):
                # tushare pro uses query(api_name, **kwargs) or api_name(**kwargs)
                # Usually tushare.pro_api().query(api_name, **kwargs)
                return api.query(api_name, **kwargs)
            elif hasattr(api, api_name):
                method = getattr(api, api_name)
                return method(**kwargs)
            else:
                logger.warning(f"Tushare: API {api_name} not found")
                return None
        except Exception as e:
            logger.error(f"Tushare: Query {api_name} failed: {e}")
            return None
